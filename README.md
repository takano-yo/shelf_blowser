...
`books.json` を **2 つの経路**で用意する。**表示層（`site/`）はどちらの経路から
来た `books.json` でもそのまま描画できる**（= 表示層を一切変えずに動的化できる）。
...
| [`site/`](site/README.md) | 本棚 UI・検索窓・詳細オーバーレイ（バニラ JS・ビルド工程なし） | `books.json`（静的 or API） |
| [`fetch/`](fetch/README.md) | （将来・バッチ）詳細 API から件名・典拠著者・分類を取得 | 一件ごとの詳細 API |
...
│   └── data/books.json       #   build の出力（既定表示・フォールバック）
├── fetch/                    # 将来のバッチ（詳細検索用）
...
今後の作業を優先度順に整理する。各項目の**要件・手順は各モジュールの README** に定義した。
...
| # | 作業 | 要件定義の場所 |
|---|---|---|
| 3 | 正規化ロジックのユニットテストと CI（回帰防止） | [core/README.md](core/README.md) |
| 4 | server の運用堅牢化（gzip・キャッシュ上限・レート制限・入力検証） | [server/README.md](server/README.md) |
| 5 | build の整列処理の重複解消（`core.normalize_items` へ一本化） | [build/README.md](build/README.md) |
| 6 | LICENSE の追加（下記） | 本書 |

### P3 — UX 改善（仮想書架の先行例に学ぶ）

| # | 作業 | 要件定義の場所 |
|---|---|---|
| 7 | URL 状態同期（検索語・タブ・並べ替えの共有/ブックマーク/戻る対応） | [site/README.md](site/README.md) |
| 8 | 検索リクエストの競合対策（AbortController） | [site/README.md](site/README.md) |
| 9 | ピボットブラウジング（著者・出版社・シリーズのクリックで棚を再生成） | [site/README.md](site/README.md) |
| 10 | 詳細オーバーレイに「棚の近傍」表示（virtual browse） | [site/README.md](site/README.md) |
| 11 | アクセシビリティ強化（タブの矢印キー移動・フォーカストラップ） | [site/README.md](site/README.md) |

### P4 — 保留中の将来機能

| # | 作業 | 要件定義の場所 |
|---|---|---|
| 12 | fetch（詳細書誌のバッチ取得）の実装 | [fetch/README.md](fetch/README.md) |
| 13 | NDC 分類順の棚（請求記号ブラウジングの本来形） | [fetch/README.md](fetch/README.md)・[site/README.md](site/README.md) |
| 14 | 件名・典拠著者・分類のファセット検索 | [build/README.md](build/README.md)・[site/README.md](site/README.md) |
| 15 | 既定データの定期更新（GitHub Actions での再取得・再ビルド） | [build/README.md](build/README.md) |
