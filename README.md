...
> NDC 分類名は日本図書館協会（JLA）が CC-BY で公開する
> [日本十進分類法 新訂9版データ（NDC-LD）](https://www.jla.or.jp/committees/bunrui/ndc-data/)
> を利用しています。
>
...
- **入口はスタートページ**（計画・P2）: 検索語または NDC 分類（類目→綱目→細目の
  ...
  分類名は JLA 公式の [NDC 新訂9版データ（NDC-LD・CC BY）](https://www.jla.or.jp/committees/bunrui/ndc-data/)
  を出典表示つきで使用する（NDC10 版は転載許諾が得られた場合に差し替え）。
...
| # | 作業 | 要件定義の場所 |
|---|---|---|
| 3 | 事前調査（分類検索 API の確認 → **確認済み**・NDC 分類名の利用許諾 → 未・全分類の件数実測 → 手段は `ndc_fetch.py --counts` として整備済み） | [docs/site-structure.md](docs/site-structure.md)・[core/README.md](core/README.md) |
| 4 | NDC マスタ整備（最大 1,110 分類の記号・分類名・出典 meta。**完了**: 分類名は JLA 公式 NDC9 版データ〈CC-BY〉を `fetch/ndc_labels.py` で収録） | [docs/site-structure.md](docs/site-structure.md)・[build/README.md](build/README.md) |
| 5 | NDC ごとの棚データ取得バッチと定期更新（**バッチ・ビルドは実装済み**。全分類の取得実行と定期更新の運用が残作業） | [fetch/README.md](fetch/README.md)・[build/README.md](build/README.md) |
...
実測・確定は [docs/site-search-poc.md](docs/site-search-poc.md)。
...
**実装済み**（PoC → 正規化/コーパス生成 → `/api/ping` → site 実装 → デプロイ時生成）。

| # | 作業 | 状態 | 要件定義の場所 |
|---|---|---|---|
| 20 | 検索コーパス生成（`build.py --search-index`・正規化の共有テストベクタ・Pages デプロイ時生成） | **完了** | [docs/site-search.md](docs/site-search.md)・[build/README.md](build/README.md) |
| 21 | 検索モード切替 UI・類内検索・`/api/ping`（site / server） | **完了** | [docs/site-search.md](docs/site-search.md)・[site/README.md](site/README.md)・[server/README.md](server/README.md) |
...
| # | 作業 | 要件定義の場所 |
|---|---|---|
| 13 | 検索リクエストの競合対策（AbortController）→ **P2.5 #21 の実装へ統合・完了**（`site/js/search.js`） | [site/README.md](site/README.md)・[docs/site-search.md](docs/site-search.md) |
...
- データ・画像は各提供元の条件に従う旨を README に明記する:
  ...
  NDC 分類名（NDC-LD・NDC9 版）＝ CC BY（提供元: 日本図書館協会）。
