...
- **`"personal"`（単著/共著）**: 第 1 寄与者の役割が `著`・`共著`・`執筆`・`著者`
  ...
  **ただし第 1 グループに「ほか／他」省略表記を含む場合は除外**（下記参照）。
- **`"editorial"`（編集書）**: 上記以外。第 1 寄与者が `編`・`編著`・`訳`・`校注`・
  `編集委員` など著以外（例: `井上俊 [ほか] 編`、`久松潜一校注 . 西尾實校注`）。
  **`creatorRaw` が空（著者表記なし）も editorial** とする。
- **「ほか／他」省略表記を含むもの**（例: `著者名[ほか]著`・`福田恆存 [ほか] 著`・
  `伊豆利彦他著`）。これは「列挙した以外にも著者が居る」＝多数著者のまとめを意味する
  ため、筆頭の役割が `著` 系でも **editorial** とする。
...
2. 役割グループ区切り（`" ; "` / `" . "`）で分け、**第 1 グループ**だけを見る
   ...
   役割として扱う）。
3. **第 1 グループに「ほか／他」省略表記が含まれれば `editorial`**
   （`has_others_marker()`。下記「ほか／他の検出」）。
4. 第 1 グループの**末尾の役割語を最長一致**で取り出す（角括弧内 `[著]`・
   `[ほか編集]` や、`[ほか]著` のように括弧の外側にある役割も解決する）。
   先頭属性 `[著者] 名` は先頭の属性を役割とみなす。
5. 役割が `{著, 共著, 執筆, 著者}` のいずれかなら `personal`、それ以外は `editorial`。
...
> （「ほか／他」省略表記の反映により、従来 `personal` だった 116 件が `editorial`
> に移動した。誤検出は `[岡野他家夫著]` を含め 0 件であることを全件で確認済み。）
