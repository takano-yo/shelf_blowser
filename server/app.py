...
提供するもの:
  GET /api/ping                              … サーバ稼働判定用（{"ok": true} を返すだけ。
                                               site が読込時に叩き、失敗時は API 検索を
                                               グレーアウトする → docs/site-search.md）
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == "/api/ping":
            # サーバ稼働判定用の軽量エンドポイント。site は読込時に 1 回 fetch し、
            # 成功なら CiNii API 検索モードを有効化、失敗（404・ネットワーク・タイム
            # アウト）なら API 検索をグレーアウトする（docs/site-search.md）。
