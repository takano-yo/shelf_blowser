...
役割は「検索語 → OpenSearch 取得 → core.normalize で正規化・整列 → books 配列を
JSON で返す」だけ。返す JSON は build が出力する site/data/books.json と同一
スキーマ（レコード配列）で、フロント（site/js/app.js）はこれをそのまま
buildShelfItems() に渡せる ── 表示層を一切変えずに動的化できる、という設計の要。

提供するもの:
  GET /api/search?q=<キーワード>&count=<N>   … 動的検索結果（books 配列）
...
検索語ごとの結果は cache/ に保存し、同じ語の再取得を避ける（CiNii へのマナー・
性能）。build の OpenBD キャッシュと同じ「冪等・再開可能」の思想。標準ライブラリのみ。
"""
...
def _cache_path(query, count):
    key = hashlib.sha1(f"{CONFIG['live']}|{count}|{query}".encode("utf-8")).hexdigest()
